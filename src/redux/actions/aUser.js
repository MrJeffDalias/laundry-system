...
export const GetInfoUser = createAsyncThunk('user/GetInfoUser', async (headers, { dispatch }) => {
  try {
    const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/lava-ya/get-user`, headers);
    ...
    if (error.response.status === 500) {
      ...
      if (error.response.status === 403) {
        if (error.response.data.type === 'outTime') {
          await dispatch(
            messageGeneral({ title: 'Comunicado', message: error.response.data.mensaje, ico: 'time-out' })
          );
        } else if (error.response.data.type === 'locking') {
          await dispatch(
            messageGeneral({ title: 'Comunicado', message: error.response.data.mensaje, ico: 'close-emergency' })
          );
        } else {
...
export const GetListUser = createAsyncThunk('user/GetListUser', async () => {
  try {
    const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/lava-ya/get-list-users`);
    return response.data;
  } catch (error) {
